/*
# Revert in-app welcome message, keep only order notifications

## Overview
The welcome message should be sent via EMAIL, not in-app chat. This migration
removes the send_general_welcome_message function and the backward-compatible
wrapper, keeping only the order flow notification triggers.

## Changes
1. Drop send_general_welcome_message function
2. Restore send_seller_welcome_message to its original form (seller verification
   instructions in admin chat — this is separate from the email welcome)
3. Keep order notification triggers unchanged
4. Keep message_type columns unchanged
*/

-- Drop the general welcome message function
DROP FUNCTION IF EXISTS send_general_welcome_message(uuid);

-- Restore the original send_seller_welcome_message function
