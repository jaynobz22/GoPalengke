/*
# Add Video Call Credits System

## Purpose
Implements a credit-based system for video calling. Users get 3 free credits
to start, and can purchase more via manual QR-code payment (admin-approved).

## Changes

### 1. profiles table — new column
- `video_credits` (integer, NOT NULL, DEFAULT 3): Each user starts with 3 free
  video call credits. Decremented by 1 every 60 seconds during an active call.

### 2. New table: video_credit_purchases
Tracks user top-up requests. Modeled after the existing fee_payments table.
- `id` (uuid, primary key)
- `user_id` (uuid, NOT NULL, DEFAULT auth.uid(), references profiles)
- `credits` (integer, NOT NULL): Number of credits in the package (10, 30, or 70)
- `amount_paid` (numeric, NOT NULL): Price paid in PHP (10, 25, or 50)
- `reference_number` (text, NOT NULL): Payment reference from e-wallet
- `screenshot_url` (text): URL of uploaded payment screenshot
- `status` (text, NOT NULL, DEFAULT 'pending'): pending / approved / rejected
- `approved_by` (uuid, nullable): Admin user ID who approved
- `approved_at` (timestamptz, nullable): Timestamp of approval
- `created_at` (timestamptz, DEFAULT now())
- `updated_at` (timestamptz, DEFAULT now())

### 3. Security (RLS)
- Enable RLS on video_credit_purchases.
- Users can SELECT only their own purchase records.
- Users can INSERT only their own purchase records (auth.uid() = user_id).
- Admins can SELECT all records and UPDATE status (approve/reject).
- Profiles: users can already read their own profile (existing policy covers
  the new video_credits column). No new profile policy needed.

### 4. SECURITY DEFINER functions

#### deduct_video_credit()
Atomically decrements the caller's video_credits by 1 and returns the new
balance. Returns -1 if the user has 0 credits (should not happen if the
frontend checks beforehand, but acts as a server-side safety net).
Callable by any authenticated user on their own account.

#### approve_video_credit_purchase(purchase_id uuid)
Called by admin to approve a pending purchase. Adds the purchased credits to
the user's video_credits balance and marks the purchase as approved.
Only callable by users with role = 'admin'.

### 5. Storage bucket
- `payment-screenshots`: public bucket for uploading payment screenshot images.

## Important Notes
1. The deduct_video_credit function is atomic — two concurrent calls cannot
   double-spend the same credit.
2. The approve function checks admin role via raw_app_meta_data, not
   user_metadata (which is user-editable).
3. Existing profiles get video_credits = 3 via the column DEFAULT.
*/
